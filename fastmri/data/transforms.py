...
class DataTransform(object):
    ...
    def __call__(self, kspace, mask, target, attrs, fname, slice_num):
        ...
        abs_image = fastmri.complex_abs(image)
        mean = torch.tensor(0.0)
        std = abs_image.std()

        img_full = normalize(image, mean, std)
        k_full = fastmri.fft2c(img_full)

        seed = None if not self.use_seed else tuple(map(ord, fname))
        k_sub, mask = apply_mask(k_full, self.mask_func, seed)
        mask = mask.repeat(k_full.shape[0], 1, 1)

        img_sub = fastmri.ifft2c(k_sub)

        return img_sub.permute(2, 0, 1), img_full.permute(2, 0, 1), mean, std, os.path.basename(fname), \
               slice_num, k_sub, mask
