...
def fft2c(data):
    ...
    data = torch.fft.fft2(torch.view_as_complex(data), norm='ortho')
...
def ifft2c(data):
    ...
    assert data.size(-1) == 2
    data = ifftshift(data, dim=(-3, -2))
    data = torch.fft.ifft2(torch.view_as_complex(data), norm='ortho')
    data= torch.view_as_real(data)
    data = fftshift(data, dim=(-3, -2))
...
def complex_abs_eval(data):
    ...
    return np.sqrt((data[:, 0:1, :, :] ** 2 + data[:, 1:2, :, :] ** 2))
...
class DataConsistency(torch.nn.Module):
