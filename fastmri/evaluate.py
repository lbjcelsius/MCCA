...
class DistributedMetricSum(Metric):
    def __init__(self, name=None, dist_sync_on_step=True):
        super().__init__(dist_sync_on_step=dist_sync_on_step)

        self.add_state(name='quantity', default=torch.tensor(0.0), dist_reduce_fx="sum")
...
def ssim(gt, pred, maxval=None):
    ...
    ssim = structural_similarity(gt, pred, channel_axis=0, data_range=maxval)
